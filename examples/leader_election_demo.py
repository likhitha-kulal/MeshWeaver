...
def render_dashboard(nodes, iteration_desc=""):
    ...
    for n in nodes:
        ...
        role_badge = f"👑 {metrics.role}" if metrics.is_leader else f"   {metrics.role}"
...
async def main():
    ...
    try:
        ...
        print("🗳️  Triggering Candidate Election from Node 1...")
        ...
        if active_leader:
            ...
            if new_leader:
                print(f"✅ Success! New Leader {new_leader.node_id.hex()[:8]} has smoothly taken over leadership.")
