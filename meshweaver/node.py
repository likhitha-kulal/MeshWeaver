...
class MeshNode:
    ...
    def _get_active_peers_for_consensus(self) -> List[Tuple[str, int]]:
        peers = []
        for p in self.gossip_manager.get_all_peers().values():
            peers.append((p.ip, p.udp_port))
        for host, port in self.gossip_manager.neighbors.values():
            peers.append((host, port))
        for bucket in self.routing_table.buckets:
            for contact in bucket.contacts:
                peers.append((contact.ip, contact.udp_port))
        return list({(ip, port) for ip, port in peers if (ip, port) != (self.host, self.bound_udp_port)})
    ...
    def _get_term_and_role(self) -> Tuple[int, str]:
        return self.leader_election.current_term, self.leader_election.role.value

    # --- Replicated State Machine & Distributed Locking APIs ---
