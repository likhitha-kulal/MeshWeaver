...
class TestNetworking(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_store_and_find_value_rpc(self):
        node_a = MeshNode(host="127.0.0.1", udp_port=18030, tcp_port=18031)
        node_b = MeshNode(host="127.0.0.1", udp_port=18032, tcp_port=18033)
        fallback = NodeInfo(NodeID(), "127.0.0.1", 18034)
        node_b.routing_table.add_contact(fallback)

        await node_a.start()
        await node_b.start()

        try:
            self.assertIsNotNone(node_a.udp_protocol)
            self.assertIsNotNone(node_b.udp_protocol)
            assert node_a.udp_protocol is not None
            assert node_b.udp_protocol is not None

            stored = await node_a.udp_protocol.send_store(
                "127.0.0.1",
                node_b.bound_udp_port,
                "answer",
                42,
            ...
            found = await node_a.udp_protocol.send_find_value(
                "127.0.0.1",
                node_b.bound_udp_port,
                "answer",
                timeout=3.0,
            )
            self.assertEqual(found, {"found": True, "value": 42})

            node_b.udp_protocol.local_store["expired"] = ("old", 0.0)
            missing = await node_a.udp_protocol.send_find_value(
                "127.0.0.1",
                node_b.bound_udp_port,
                "expired",
                timeout=3.0,
            ...
            self.assertIn(fallback.to_dict(), missing["nodes"])
        finally:
            await node_a.stop()
            await node_b.stop()
